...
struct CountTrainWebViewBox: UIViewControllerRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: CountTrainWebLoader
    var defaultOrientations: UIInterfaceOrientationMask = .landscape

    // MARK: - Координатор

    func makeCoordinator() -> CountTrainWebCoordinator {
        CountTrainWebCoordinator { [weak loader] status in
    ...
    func makeUIViewController(context: Context) -> CountTrainBaseWebViewController {
        let configuration = createWebViewConfiguration()
        return CountTrainBaseWebViewController(
    ...
    func updateUIViewController(_ uiViewController: CountTrainBaseWebViewController, context: Context) {
