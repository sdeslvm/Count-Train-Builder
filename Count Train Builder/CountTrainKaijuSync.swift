...
protocol CountTrainLoadStateRepresentable {
    var type: CountTrainLoadState.StateType { get }
...
struct CountTrainLoadState: Equatable, CountTrainLoadStateRepresentable {
    ...
    static func idle() -> CountTrainLoadState {
        CountTrainLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> CountTrainLoadState {
        CountTrainLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> CountTrainLoadState {
        CountTrainLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> CountTrainLoadState {
        CountTrainLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> CountTrainLoadState {
        CountTrainLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: CountTrainLoadState) -> Bool {
    ...
    static func == (lhs: CountTrainLoadState, rhs: CountTrainLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension CountTrainLoadState: CustomStringConvertible {
