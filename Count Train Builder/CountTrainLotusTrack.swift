...
struct CountTrainGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://counttrainbld.pro/log")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            CountTrainEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}
