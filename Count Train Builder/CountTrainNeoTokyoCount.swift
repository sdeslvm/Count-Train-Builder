...
protocol WebLoadable: AnyObject {
    var state: CountTrainWebStatus { get set }
...
final class CountTrainWebLoader: NSObject, ObservableObject, WebLoadable, ProgressMonitoring {
    // MARK: - Свойства

  @Published var state: CountTrainWebStatus = .standby
    ...
    func observeProgression() {
        progressPublisher
            ...
            .sink { [weak self] progress in
                ...
                let newState: CountTrainWebStatus = progress < 1.0 ? .progressing(progress: progress) : .finished
...
extension CountTrainWebLoader: WKNavigationDelegate {
...
extension CountTrainWebLoader {
    ...
    func publish(_ newState: CountTrainWebStatus) {
...
private extension CountTrainWebLoader {
    func updateState(_ newState: CountTrainWebStatus) {
